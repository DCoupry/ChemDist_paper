...
def chemdist_func(s, p=0.0):
    ...
    natoms = g.number_of_nodes()
    todel = []
    for n in range(natoms):
        if numpy.random.rand() < p:
            todel.append(n)
    g.remove_nodes(todel)
...
def plot(df):
    ...
    df.plot("P(node drop)", "Dataset average", color="k", ax=ax)
    ax.fill_between(df["P(node drop)"], df["Q25"], df["Q75"], alpha=0.2, color="k", label="50% CI")
    ax.legend(frameon=False, loc=4)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.set_xlabel("Node drop probability")
...
def main(args):
    ...
    odf["P(node drop)"] = drop_probas
    print(odf)
    odf.to_csv("deletions_results_ablation_nodes.csv")
    return
